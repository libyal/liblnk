...
{
	static char *function = "liblnk_guid_to_string";
	int print_count       = 0;
	...
	if( string_size < LIBLNK_GUID_STRING_SIZE )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	 ...
	 * The GUID is stored as uint32 - uint16 - uint16 - 8 byte array
	 */
	if( byte_order == LIBLNK_ENDIAN_BIG )
	{
	...
	else if( byte_order == LIBLNK_ENDIAN_LITTLE )
	{
		print_count = narrow_string_snprintf(
			       (char *) string,
			       string_size,
